...
def simulate_serving(
    ...
    """
    ttft = prefill_seconds(model, prompt_tokens, gpu, precision, utilization, cache_hit_fraction)
    avg_kv_tokens = prompt_tokens + output_tokens / 2  # rough mean sequence length across decode
    decode_step = decode_step_seconds(
        model, gpu, precision, decode_batch_size, avg_kv_tokens, paged_attention, block_size
    )
...
def estimate_serving_capacity(
    ...
    """
    usable_bytes = gpu.vram_gb * 1e9 * gpu_memory_utilization
    w_bytes = weights_bytes(model, precision)
    kv_budget_bytes = max(0.0, usable_bytes - w_bytes)
    per_seq_bytes = kv_cache_bytes_per_token(model, precision) * avg_seq_len
    max_sequences = int(kv_budget_bytes // per_seq_bytes) if per_seq_bytes > 0 else 0

    return CapacityEstimate(
        usable_vram_gb=usable_bytes / 1e9,
        weights_gb=w_bytes / 1e9,
        kv_budget_gb=kv_budget_bytes / 1e9,
